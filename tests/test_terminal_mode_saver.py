...
def restore(*escapes: bytes) -> bytes:
    ...
    return saver.get_restore_escapes()
...
def test_non_style_escapes_ignored():
    ...
    assert restore(b"\x1b[2J", b"\x1b[H", b"\x1b[10;5H") == b""
...
def test_dec_save_and_restore():
    ...
    assert saver.get_restore_escapes() == b"\x1b[1;31m"
    saver.add_escape(b"\x1b8")  # DECRC: back to just bold
    assert saver.get_restore_escapes() == b"\x1b[1m"
...
def test_sgr_and_modes_coexist():
    ...
    assert out == b"\x1b[1m\x1b[?25l\x1b[4h"


def test_skipped_dec_modes_not_captured():
    # 2 (DECANM), 3 (DECCOLM), 1048 (cursor save) lack restorable boolean state
    assert restore(b"\x1b[?2l") == b""
    assert restore(b"\x1b[?3h") == b""
    assert restore(b"\x1b[?1048h") == b""
...
def test_mouse_protocol_modes_replay_in_order():
    ...
    assert restore(b"\x1b[?1000h", b"\x1b[?1003l") == b"\x1b[?1000h\x1b[?1003l"
...
def test_mode_runs_follow_set_order_not_action():
    ...
    assert out == b"\x1b[?1h\x1b[?7l\x1b[?25h"
...
def test_xtsave_and_xtrestore_dec_modes():
    ...
    assert restore(b"\x1b[?25h", b"\x1b[?25r") == b""
...
def test_charset_designation_per_slot():
    ...
    assert restore(b"\x1b(0", b"\x1b)B") == b"\x1b(0\x1b)B"
...
def test_charset_locking_shift():
    ...
    assert restore(b"\x1b)0", b"\x0e") == b"\x1b)0\x0e"
...
def test_charset_and_keypad_ordering():
    ...
    assert out == b"\x1b[1m\x1b[?25l\x1b(0\x1b="
...
def test_leds():
    ...
    assert restore(b"\x1b[1q", b"\x1b[22q") == b"\x1b[1q\x1b[22q"  # +num, -caps
    assert restore(b"\x1b[1q", b"\x1b[22q", b"\x1b[21q") == b"\x1b[22q\x1b[21q"
    # LEDs survive a soft reset (only RIS clears them), so replay after DECSTR
    assert restore(b"\x1b[1q", b"\x1b[!p") == b"\x1b[!p\x1b[1q"
...
def test_cursor_style():
    ...
    assert restore(b"\x1b[3 q", b"\x1b[!p") == b"\x1b[!p\x1b[3 q"
...
def test_xterm_pointer_mode():
    ...
    assert restore(b"\x1b[>2p", b"\x1b[!p") == b"\x1b[!p\x1b[>2p"
