...
def test_match_none_and_empty():
    ports = [
        ok_serial.PortInfo(name="a", attr={"x": "1"}),
        ok_serial.PortInfo(name="b", attr={}),
    ]
...
def test_match_callable_passthrough():
    ports = [
        ok_serial.PortInfo(name="a", attr={"x": "1"}),
        ok_serial.PortInfo(name="b", attr={"x": "2"}),
    ]
...
def test_whole_word_match():
    ports = [
        ok_serial.PortInfo(name="a", attr={"device": "/dev/ttyS1"}),
        ok_serial.PortInfo(name="b", attr={"device": "/dev/ttyS10"}),
    ]
...
def test_wildcards():
    ports = [
        ok_serial.PortInfo(name="a", attr={"serial_number": "DF62585783"}),
        ok_serial.PortInfo(name="b", attr={"serial_number": "AB12345678"}),
    ]
...
def test_word_boundaries_around_punctuation():
    ...
    ports = [
        ok_serial.PortInfo(name="a", attr={"vid_pid": "2e8a:0005"}),
    ]
