"""Unit tests for ok_serial._monitor.ok_serial.SerialConnectionMonitor."""
...
def test_connect_sync_no_match_times_out(pty_serial, set_scan_override):
    ...
    with ok_serial.SerialConnectionMonitor("nomatch") as monitor:
        conn = monitor.connect_sync(timeout=0.2)
        assert conn is None
...
def test_connect_sync_reuses_connection(pty_serial, set_scan_override):
    ...
    with ok_serial.SerialConnectionMonitor("test") as monitor:
        ...
        conn2 = monitor.connect_sync(timeout=1)
...
def test_connect_sync_waits_for_port_to_appear(pty_serial, set_scan_override):
    ...
    thread.start()
    opts = ok_serial.SerialMonitorOptions(scan_interval=0.05)
    with ok_serial.SerialConnectionMonitor("delayed", mopts=opts) as monitor:
        conn = monitor.connect_sync(timeout=2)
        assert conn is not None
        assert conn.port_name == pty_serial.path
    thread.join()
...
def test_scan_timeout_raises_exhausted(set_scan_override):
    ...
    opts = ok_serial.SerialMonitorOptions(scan_interval=0.05, scan_timeout=0.2)
    with ok_serial.SerialConnectionMonitor("never", mopts=opts) as monitor:
        with pytest.raises(ok_serial.SerialMonitorExhausted):
            monitor.connect_sync(timeout=2)
...
def test_scan_timeout_not_hit_when_port_present(pty_serial, set_scan_override):
    ...
    opts = ok_serial.SerialMonitorOptions(scan_interval=0.05, scan_timeout=0.2)
    with ok_serial.SerialConnectionMonitor("prompt", mopts=opts) as monitor:
        conn = monitor.connect_sync(timeout=1)
        assert conn is not None
        assert conn.port_name == pty_serial.path
...
def test_multiple_matches_does_not_connect(pty_serial, set_scan_override):
    ...
    opts = ok_serial.SerialMonitorOptions(scan_interval=0.05)
    with ok_serial.SerialConnectionMonitor("dup", mopts=opts) as monitor:
        # Ambiguous match: refuse to pick a port, time out with no connection.
        assert monitor.connect_sync(timeout=0.3) is None
...
def test_multiple_matches_resolved_then_connects(pty_serial, set_scan_override):
    ...
    opts = ok_serial.SerialMonitorOptions(scan_interval=0.05)
    with ok_serial.SerialConnectionMonitor("dup", mopts=opts) as monitor:
        assert monitor.connect_sync(timeout=0.2) is None
...
async def test_connect_async_finds_port(pty_serial, set_scan_override):
    ...
    with ok_serial.SerialConnectionMonitor("async_test") as monitor:
        conn = await asyncio.wait_for(monitor.connect_async(), timeout=1)
        assert conn.port_name == pty_serial.path
...
async def test_connect_async_waits_for_port(pty_serial, set_scan_override):
    ...
    opts = ok_serial.SerialMonitorOptions(scan_interval=0.05)
    with ok_serial.SerialConnectionMonitor("appears", mopts=opts) as monitor:
        asyncio.create_task(add_port_later())
        conn = await asyncio.wait_for(monitor.connect_async(), timeout=2)
        assert conn.port_name == pty_serial.path
