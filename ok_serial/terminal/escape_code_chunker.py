import re


class EscapeCodeChunker:
    """Breaks VTxxx data into output characters and control sequences."""

    PARSE_RX = re.compile(
        # group 1: string of complete UTF-8 code points
        # https://en.wikipedia.org/wiki/UTF-8#Description
        b"((?:"
        b"[\x20-\x7e]|"
        b"[\xc0-\xdf][\x80-\xbf]|"
        b"[\xe0-\xef][\x80-\xbf]{2}|"
        b"[\xf0-\xf7][\x80-\xbf]{3}"
        b")*)|"
        # group 2: *incomplete* UTF-8 code point at end of data
        b"([\xc0-\xf7][\x80-\xbf]*$)|"
        # group 3: one complete VTxxx CSI control sequence
        # https://vt100.net/emu/dec_ansi_parser (not incl. DCS, OSC, etc)
        b"((?:\x9b|\x1b\\[)[\x20-\x3f]*[\x40-\x7e])|"
        # group 4: *incomplete* VTxxx CSI control sequence at end of data
        b"(\1b|\x1b\\[\x20-\x3f]*$|\x9b[\x20-\x3f]*$)|"
        # group 5: any other byte (control code, invalid, etc)
        b"[\x00-\xff]"
    )

    TIMEOUT = 0.1  # seconds to pause before giving up on partial data

    def __init__(self):
        self._partial = bytearray()
        self._partial_timeout = 0.0
        self._chunks: list[str | bytes] = []

    def add_data(self, data: bytes, data_time: float):
        """Adds terminal data with time of arrival."""

        if not data and self._partial and data_time > self._partial_timeout:
            self._chunks.append(bytes(self._partial[:1]))
            self._partial = self._partial[1:]

        self._partial.extend(data)
        pos = 0
        while pos < len(self._partial):
            match = self.PARSE_RX.match(self._partial, pos)
            assert match, self._partial[pos:]
            chars, char_part, esc, esc_part, other = match.groups()
            if chars:
                self._chunks.append(chars.decode())
                pos += len(chars)
            elif esc:
                self._chunks.append(esc)
                pos += len(esc)
            elif other:
                self._chunks.append(other)
                assert len(other) == 1, other
                pos += 1
            else:
                assert self._partial in (char_part, esc_part)
                self._partial_timeout = data_time + self.TIMEOUT
                return

        self._partial.clear()
        self._partial_timeout = 0.0

    def get_chunks(self) -> list[str | bytes]:
        """Gets accumulated data broken into chunks:
        - str for characters to be output
        - bytes for control sequences to be interpreted
        """
        out, self._chunks = self._chunks, []
        return out
